...
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (45567179,316139,45581779,45576879)

) I
) C UNION ALL
SELECT 1 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (9202,9201)

) I
) C UNION ALL
SELECT 2 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (45567179,319835,316139,45581779,45576879)
...
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (9202,9201)
  and c.invalid_reason is null
...
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (45567179,319835,316139,45581779,45576879,319835,316139,439694,439696,314378,444101,439846)

) I
) C UNION ALL
SELECT 6 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (43533372,2721439)
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,0,P.START_DATE) AND A.START_DATE <= DATEADD(day,1827,P.START_DATE) ) cc
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,0,P.START_DATE) AND A.START_DATE <= DATEADD(day,1827,P.START_DATE) ) cc on p.person_id = cc.person_id and p.event_id = cc.event_id
...
select event_id, person_id,
  case when DATEADD(day,1827,start_date) > op_end_date then op_end_date else DATEADD(day,1827,start_date) end as end_date
INTO #strategy_ends
